/** @import { OrdinalExpression } from './types.js' */
/** @import { OrdinalMarkupConfig } from './config.js'*/
...
export class OrdinalMarkup {
    constructor() {
        ...
        this.btn.appendChild(document.createTextNode("+1"));
        ...
        this.ordinal = {type: 'number', value: 0};
        ...
        this.btnMaximize.addEventListener('click', event => {
            const m = maximizeOne(this.ordinal, validateBase(Number(this.baseInput.value))[0]);
            if (m !== null) {
                this.ordinal = m;
                this._updateDisplay();
            }
    ...
    _maximize() {
        if (this.maximizerSwitch.checked) {
            const m = maximize(this.ordinal, validateBase(Number(this.baseInput.value))[0]);
            if (m !== null) this.ordinal = m;
        }
    }
    _updateDisplay() {
        this.ordinalDisplay.replaceChildren(ordinalToMathML(this.ordinal));
    }
    /**
