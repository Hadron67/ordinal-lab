...
export class App {
    ...
    constructor(root) {
        ...
        const btnTablist = button('\u03A9' /* \Omega */, () => {
            if (this.appletTabList.isConnected) {
                this.appletTabList.remove();
                btnTablist.classList.remove('selected');
            } else if (btnTablist.parentElement) {
                this.header.prepend(this.appletTabList);
                btnTablist.parentElement.insertAdjacentElement('afterend', this.appletTabList);
                btnTablist.classList.add('selected');
            }
        });
        btnTablist.classList.add('tablist');
        ...
        this.header.append(html('div', btnTablist), tabHeaderContainer, tabOtherContainer);
