...
import { button, toggleButton } from "./domutils.js";
...
export class OrdinalListExpander {
     ...
     */
    onCreate(applet) {
        if (applet.entry && applet.entry.tabHeader) {
            const btnBookmarks = button('Bookmarks', () => {
                if (this.bookmarks.root.isConnected) {
                    this.bookmarks.root.remove();
                    btnBookmarks.classList.remove('selected');
                } else {
                    this.root.appendChild(this.bookmarks.root);
                    btnBookmarks.classList.add('selected');
                }
            });
            applet.entry.tabHeader.appendChild(btnBookmarks);
        }
    }
    getRoot() {
        return this.root;
    ...
    serialize(data) {
        ...
        data.showBookmarks = this.bookmarks.root.parentNode !== null;
    }
    /**
     * @param {any} data
     */
