...
export class OrdinalExpanderItem {
    ...
    renderOrdinal() {
        this.ordinalContainer.replaceChildren(this.ordinalManager.render(this.ordinal));
    }
    expandOne() {
        ...
        this.btnExpandCadidates.addEventListener('click', () => {
            if (this.expansionCadidatesList.isConnected) {
                ...
                for (const ord of this.expansionCadidates) {
                    const elem = li(this.expanderItem.expander.config.render(ord));
...
export class Y0SequenceOrdinalListExpanderElementManager {
    ...
    constructor(imageConfig, elem) {
        ...
        this.btnShowMontagne = document.createElement('button');
        this.btnShowMontagne.type = 'button';
        this.btnShowMontagne.textContent = 'Montagne';
        this.btnShowMontagne.addEventListener('click', function() {
            this.classList.toggle('selected');
            elem.renderOrdinal();
        });

        this.ordinalElem.menu.btns.appendChild(li(this.btnShowMontagne));
    }
    /**
     * @param {any} data
     */
    serialize(data) {
        data.showMontagne = this.btnShowMontagne.classList.contains('selected');
    }
    /**
     * @param {any} data
     */
    deserialize(data) {
        if (data.showMontagne) {
