...
import { TabEntry } from "./tabhost.js";
...
export class OrdinalListExpander {
        ...
        if (entry && entry.tabHeader) {
            ...
            const btnBookmarks = button('Bookmarks', () => {
                if (this.showBookmarks) {
                    ...
                    entry.host.tabOtherContainer.replaceChildren(this.bookmarks.root);
            ...
            entry.ondefocus.push(() => {
                if (this.showBookmarks) {
                    this.bookmarks.root.remove();
                }
            });
            entry.tabHeader.appendChild(btnBookmarks);
        }
    }
    getRoot() {
        return this.root;
    ...
    serialize(data) {
        ...
        data.showBookmarks = this.showBookmarks;
    }
    /**
     * @param {any} data
     */
    loadData(data) {
        ...
        this.showBookmarks = Boolean(data.showBookmarks);
    }
    /**
     * @param {OrdinalExpanderItem<T>} item
