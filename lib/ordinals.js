/** @import { VeblenFunction, Ordering, NestedArrayCoord, Expression, CompoundExpression, TracedExpressionWithIndex, Omega, OrdinalCollapsingFn, ExpressionType, OrdinalLabOrdinal, Slot, OCFVariant, AdmissibleOmega, PowerExpression } from './types.js' */
...
function compareAsyncOne(expr1, expr2, cb, evaluator) {
    switch (expr1.type) {
        ...
        case 'ocf': switch (expr2.type) {
            ...
            case 'omega-n': {
                compareAsync(expr1.subscript, expr2, (ret, e) => e.setReturn(ret === 0 ? -1 : ret, cb), evaluator);
                break;
            }
            default: unreachable();
        }
        case 'omega-n': {
            if (expr2.type === 'omega-n') {
                compareAsync(expr1.subscript, expr2.subscript, cb, evaluator);
            } else {
...
export function evaluateAdmissibleOmega(subscript) {
    if (subscript === 0) {
        return OMEGA;
    } else return {type: 'omega-n', subscript};
}
    ...
    out: while (true) {
        if (typeof expr === 'number') {
            ...
            case 'olo':
                ...
                expr = {type: 'ocf', variant: 'b', subscript: {type: 'omega-n', subscript: 1}, arg: {type: 'lambda', args: [slot1], body: {type: 'omega-n', subscript: slot(slot1)}, iterations: slot(slot0), appliedTo: 1}};
                break out;
            case 'plus': {
