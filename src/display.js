/** @import { DisplayNotation, DisplayOptions, Expression, NestedArrayExpression } from './types/ordinal.js' */
/** @import { DisplayModeSettings } from './types/app' */
 ...
 */
function convertArrayOrdinal(array, fnHead, opt, todo) {
    if (array.kw.length === 0) {
        const args = array.positional;
        if (fnHead === 'phi' && opt.showSpecialVeblenFns && args.length === 2 && typeof args[1] === 'number' && args[1] < SPECIAL_VEBLEN_FN_NAMES.length + 1) {
            ...
            todo.push(stack => {
                ...
                const ret = [];
                if (fnHead.length > 0) {
                    ret.push(mi(fnHead === 'phi' ? '\u03C6' : fnHead), moNoStrechy('(', opt.stretchyBrackets));
                } else {
                    ret.push(mo('['));
                }
                for (let i = 0; i < len; i++) {
...
export function ordinalToDisplayNotation(ordinal, opt) {
    ...
    /**
     * Note: args are in reversed order
     * @param {string} name
        ...
        if (typeof top === 'number') {
            ...
            switch (top.type) {
                ...
                case 'i-n': {
                    ...
                    if (args.length === 1) {
                        if (args[0] === 1) {
                            stack.push([{type: 'mi', text: 'I'}]);
                        } else {
                            todo.push(stack => {
                                stack[stack.length - 1] = {type: 'sub', expr: [{type: 'mi', text: 'I'}], subscript: stack[stack.length - 1]};
                            }, args[0]);
                        }
                    } else {
                        handleFunction("I", top.args);
                    }
                    break;
                }
                case 'ocf': {
 ...
 * @param {DisplayModeSettings} opt
 */
export function displayNotationToLaTeX(ord, opt) {
    /** @type {string[]} */
    const stack = [];
    /** @type {(((s: typeof stack) => void) | Expression)[]} */
    const todo = [ord];
}
